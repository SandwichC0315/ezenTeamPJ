...
<body>
    ...
    <main>
        ...
        <div class="member_QA">         
            <div id="Member_list">
              	<h4>상품 관리</h4>
              	<button id="addBtn" onclick="location.href='<%=request.getContextPath()%>/MyPage/MPProductInsert.do'">상품 추가하기</button>
                <table>
                	<thead>
	                    <tr class="t_head">
	                    	<td>상품 번호</td>
                    ...
                    <tbody>
                    	<%for (ProductVO2 vo : list){ %>
                    	<tr>
                    		<td><%=vo.getPidx() %></td>
                    		<td><%=vo.getPtitle() %></td>
                    		<td><img src="${pageContext.request.contextPath}/resources/images/<%=vo.getPimage() %>" height="200" width="200"></td>
                    		<td><%=vo.getPprice() %></td>
                    		<td><%=vo.getPvol() %></td>
                    		<td>
                    			<button onclick="location.href='<%=request.getContextPath()%>/MyPage/MPProductModify.do?pidx=<%=vo.getPidx()%>'">수정</button>
                    			<button onclick="location.href='<%=request.getContextPath()%>/MyPage/MPProductDelete.do?pidx=<%=vo.getPidx()%>'">삭제</button>
                    		</td>
                    	</tr>
						<%}%>
                    </tbody>
		...
		<ul class="btn-group pagination">
			<c:if test="${pageMaker.prev }">
				<li><a href='<c:url value="/MyPage/MPProductManage.do?page=${pageMaker.startPage-1 }"/>'>◀<i class="fa fa-chevron-left"></i></a></li>
			</c:if>

			<c:forEach begin="${pageMaker.startPage }"
				end="${pageMaker.endPage }" var="pageNum">
				<c:if test="${select ne pageNum}">
					<a href='<c:url value="/MyPage/MPProductManage.do?page=${pageNum}"/>'>${pageNum }</a>
				</c:if>

				<c:if test="${pageNum eq select }">
					<a id="bold" href='<c:url value="/MyPage/MPProductManage.do?page=${pageNum}"/>'>${pageNum }</a>
				</c:if>
			</c:forEach>

			<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
				<a href='<c:url value="/MyPage/MPProductManage.do?page=${pageMaker.endPage+1 }"/>'>▶<i class="fa fa-chevron-right"></i></a>
			</c:if>
		</ul>
	</main>
