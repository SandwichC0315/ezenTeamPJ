...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/BoardNoticeView.css"/>
	...
	<header id="header" class="deactive">
	    ...
	    <main>
	    	...
	    	<table>
	    		...
	    		<tbody>
	    			<tr>
	    				<td colspan="4" id="contentbox">${vo.BNcontent}</td>
	    			</tr>
	    	...
	    	<button id="btnlist" onclick="location.href='BoardNotice.do'">목록</button>
	    	
	    	<c:if test="${login != null}">
		    	<c:if test="${Mvo.getMgrade() == 'A'}">
			    	<div id="btn">
				    	<button onclick="location.href='BoardNoticeModify.do?BNidx=${vo.BNidx}'">수정</button>
				    	<button onclick="delfrm.submit();">삭제</button>
			    	</div>
		    	</c:if>
			</c:if>
			
			<form name="delfrm" action="BoardNoticeDelete.do" method="post">
